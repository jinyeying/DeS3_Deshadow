...
class LRSS:
    ...
    def get_loaders(self, parse_patches=True, validation='LRSS'):
        ...
        train_dataset = ShadowDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'LRSS'),
                                        n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        filelist=None,
                                        parse_patches=parse_patches)
        val_dataset = ShadowDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'LRSS'),
                                      n=self.config.training.patch_n,
                                      patch_size=self.config.data.image_size,
                                      transforms=self.transforms,
...
class ShadowDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        if filelist is None:
            ...
            gt_names += [os.path.join(os.path.join(shadow_dir, 'trainB'), i) for i in images]
